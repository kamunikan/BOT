...
msg_welcome() {
    ...
    if [ "${message_from_id[$id]}" == "$get_AdminID" ]; then
        local msg
        msg="<b> ☀ Welcome To SCVPN BOT Panel</b>\n"
...
link_voucher() {
    ...
    if [[ ${callback_query_data[$id]} == _vouchervmess ]]; then
        local msg
        msg="User : $(tr </dev/urandom -dc a-zA-Z0-9 | head -c4)\n"
        msg+="<code>Expired : $exp Days</code>\n\n"
...
req_del() {
    cat /etc/xray/user.txt >/tmp/cad.${message_from_id[$id]}
    alluser=$(cat /etc/xray/user.txt | awk '{print $1}' | sort | uniq)
    ShellBot.sendMessage --chat_id ${callback_query_from_id[$id]} \
        --text "━━━━━━━━━━━━━━━━━━━━━\n<b>🔸 Del ACCOUNT 🔸 </b>\n━━━━━━━━━━━━━━━━━━━━━\n\n$alluser\n\n━━━━━━━━━━━━━━━━━━━━━\n" \
        --parse_mode html
    ShellBot.sendMessage --chat_id ${callback_query_from_id[$id]} \
...
input_addssh() {
    ...
    local msg
    msg="━━━━━━━━━━━━━━━━━━━━━\n<b>🔐 SSH ACCOUNT INFO 🔐 </b>\n━━━━━━━━━━━━━━━━━━━━━\n"
    msg+="<b>Username : </b><code>$Login</code>\n"
    msg+="<b>Password : </b><code>$Pass</code>\n"
    msg+="<b>Expired : </b><code>$exp</code>\n"
    msg+="━━━━━━━━━━━━━━━━━━━━━\n"
    msg+="<b>Host : </b><code>${domain}</code>\n"
    msg+="<b>OpenSSH : </b><code>22</code>\n"
    msg+="<b>Dropbear:  </b><code>109, 143</code>\n"
    msg+="<b>SSH-WS : </b><code>80</code>\n"
    msg+="<b>SSH-SSL-WS : </b><code>443</code>\n"
    msg+="<b>SSL/TLS : </b><code>447, 777</code>\n"
    msg+="<b>UDPGW : </b><code>7100-7300</code>\n"
    msg+="━━━━━━━━━━━━━━━━━━━━━\n"
    msg+="<code>GET wss://who.int/ HTTP/1.1[crlf]Host: [crlf]Upgrade: websocket[crlf][crlf]</code>\n"
    msg+="━━━━━━━━━━━━━━━━━━━━━\n"
...
create_vmess() {
    ...
    req_voucher $file_user
    req_limit
    Login=SCVPN-VM`</dev/urandom tr -dc X-Z0-9 | head -c4` 
    if grep -qw "$Login" /etc/xray/user.txt; then
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            --text "User Already Exist\n" \
            --parse_mode html
        exit 1
...
create_vless() {
    ...
    Login=SCVPN-VL`</dev/urandom tr -dc X-Z0-9 | head -c4`
    ...
    exp=$(date -d +${duration}days +%Y-%m-%d)
    domain=$(cat /etc/xray/domain)
    multi="$(cat ~/log-install.txt | grep -w "XRAY  Vless TLS" | cut -d: -f2 | sed 's/ //g')"
    none=80
...
sed -i '/#vlessgrpc$/a\#& '"$Login $exp"'\
},{"id": "'""$uuid""'","email": "'""$Login""'"' /etc/xray/config.json
vlesslink1="vless://${uuid}@${domain}:$multi?path=/vless%26security=tls%26encryption=none%26type=ws#${Login}"
vlesslink2="vless://${uuid}@${domain}:$none?path=/vless%26encryption=none%26type=ws#${Login}"
vlesslink3="vless://${uuid}@${domain}:$multi?mode=gun%26security=tls%26encryption=none%26type=grpc%26serviceName=vless-grpc%26sni=bug.com#${Login}"
...
create_xtls() {
    ...
    req_voucher $file_user
    req_limit
    Login=SCVPN-XT`</dev/urandom tr -dc X-Z0-9 | head -c4`
    if grep -qw "$Login" /etc/xray/user.txt; then
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            --text "User Already Exist\n" \
            --parse_mode html
...
create_trojan() {
    ...
    req_voucher $file_user
    req_limit
    Login=SCVPN-TR`</dev/urandom tr -dc X-Z0-9 | head -c4`
    if grep -qw "$Login" /etc/xray/user.txt; then
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            --text "User Already Exist\n" \
            --parse_mode html
...
systemctl restart xray
trojanlink1="trojan://${uuid}@${domain}:${multi}?mode=gun%26security=tls%26type=grpc%26serviceName=trojan-grpc%26sni=bug.com#${user}"
trojanlink2="trojan://${uuid}@${domain}:${multi}?path=%2Ftrojan-ws%26security=tls%26host=bug.com%26type=ws%26sni=bug.com#${user}"

    systemctl restart xray
    local msg
    msg="━━━━━━━━━━━━━━━━━━━━━\n<b> 🔐 XRAY : TROJAN ACCOUNT 🔐 </b>\n━━━━━━━━━━━━━━━━━━━━━\n\n"
   msg+="<b>User :</b> <code>$user</code>\n"
    msg+="<b>Expired :</b> <code>$exp</code>\n"
   msg+="━━━━━━━━━━━━━━━━━━━━━\n"
    msg+="<b>TROJAN TLS WS</b>\n"
...
del_conf() {
    file_user=$1
    user=$(sed -n '1 p' $file_user | cut -d' ' -f1)
    if ! grep -qw "$user" /etc/xray/user.txt; then
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            --text "User does not exist\n" \
            --parse_mode html
        exit 1
    fi
    exp="$(cat /etc/xray/user.txt | grep -w "$user" | awk '{print $3}')"
    sed -i "/^### $user $exp/,/^},{/d" /etc/xray/config.json
    sed -i "/\b$user\b/d" /etc/xray/user.txt
    systemctl restart xray.service
...
ext_conf() {
    ...
    if ! grep -qw "$user" /etc/scvpn/xray/user.txt; then
        ...
        echo -e "${user}\t${uuid}\t${exp4}" >>/etc/xray/user.txt
        ...
        (
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                ...
                'OVPN (USER EXPIRED) :')
                    ...
                    msg+="<code>Expired : $exp</code>\n\n"
                    msg+="https://t.me/${get_botName}?start=ovpn_${user}_${vouch}\n"
                    msg+="Click Link To Confirm SSH Acc\n"
